...
export default function GlobalError({ error, reset }) {
  useEffect(() => {
    console.error('Unhandled Root Exception caught by Global Error Boundary:', error);
  }, [error]);

  return (
    <html lang="en">
      <body className="min-h-screen w-full flex items-center justify-center p-4 bg-[#090c15] text-white">
        <div className="w-full max-w-md p-6 rounded-3xl bg-[#0f172a] border border-cyan-500/30 shadow-2xl text-center space-y-4">
          ...
          <p className="text-xs text-gray-300 font-mono bg-black/40 p-3 rounded-xl border border-white/10 overflow-x-auto max-h-32 text-left">
            {error?.message || 'An unhandled root exception occurred.'}
          </p>
